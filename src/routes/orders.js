...
r.post('/', auth, async (req, res) => {
  ...
  try {
    ...
    const ord = await client.query(
      `INSERT INTO orders(user_id, order_number, total, payment_method, delivery_address, notes)
       VALUES($1,$2,$3,$4,$5,$6) RETURNING id`,
      [req.user.id, order_number, total, payment_method || 'cash',
    ...
    for (const item of items) {
      await client.query(
        `INSERT INTO order_items(order_id, product_id, combo_id, quantity, unit_price, extras)
         VALUES($1,$2,$3,$4,$5,$6)`,
        [order_id, item.product_id || null, item.combo_id || null,
...
r.post('/:id/proof', auth,
  ...
  async (req, res) => {
    try {
      ...
      const o = await db.query('SELECT id FROM orders WHERE id=$1', [req.params.id]);
      if (!o.rows.length) return res.status(404).json({ message: 'Orden no encontrada' });
      await db.query(
        `UPDATE orders SET payment_proof=$1, payment_status='processing', status='payment_review' WHERE id=$2`,
...
r.post('/:id/messages', auth, async (req, res) => {
  try {
    ...
    await db.query(
      'INSERT INTO order_messages(order_id, sender_role, message) VALUES($1,$2,$3)',
      [req.params.id, role, message]
    );
