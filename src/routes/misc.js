...
r.get('/admin/dashboard', auth, admin, async (req,res) => {
  try {
    ...
    res.json({
      orders:orders.rows[0], users:users.rows[0],
      opinions:opinions.rows[0], pending_payments:pending.rows[0],
...
r.get('/admin/site-config', auth, admin, async (req,res) => {
  try {
    const rows = (await db.query('SELECT config_key,config_value FROM site_config')).rows;
    const cfg = {};
    rows.forEach(x => cfg[x.config_key]=x.config_value);
    res.json(cfg);
  } catch(e) { res.status(500).json({ message: e.message }); }
});
...
r.put('/admin/site-config', auth, admin, async (req,res) => {
  try {
    for (const [k,v] of Object.entries(req.body)) {
      await db.query(
        `INSERT INTO site_config(config_key,config_value) VALUES($1,$2) ON CONFLICT(config_key) DO UPDATE SET config_value=$2`,
        [k, v??'']
      );
    }
    res.json({ ok: true });
...
r.get('/admin/users', auth, admin, async (req,res) => {
  try {
    res.json((await db.query(
      `SELECT id,full_name,username,email,phone,role,avatar,is_active,bio,created_at,
              celular,ambiente_numero,nombre_completo_real,sena_role FROM users ORDER BY created_at DESC`
    )).rows);
  } catch(e) { res.status(500).json({ message: e.message }); }
});
...
r.put('/admin/users/:id', auth, admin, async (req,res) => {
  try {
    ...
    if (!fields.length) return res.status(400).json({ message: 'Nada' });
...
r.put('/admin/opinions/:id', auth, admin, async (req,res) => {
  try {
    await db.query(`UPDATE opinions SET status=$1,admin_response=$2 WHERE id=$3`,
      [req.body.status||'read',req.body.admin_response||null,req.params.id]);
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ message: e.message }); }
});

// HAPPY CLIENTS (Guerreros)
...
r.post('/admin/happy-clients', auth, admin, async (req,res) => {
  try {
    ...
    const result = await db.query(
      `INSERT INTO happy_clients(client_name,product_bought,description,rating,photo_url,order_index,is_active) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id`,
...
r.post('/admin/coupons', auth, admin, async (req,res) => {
  try {
    ...
    const result = await db.query(
      `INSERT INTO coupons(code,description,discount_type,discount_value,min_purchase) VALUES($1,$2,$3,$4,$5) RETURNING id`,
